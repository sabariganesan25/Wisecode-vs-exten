...
const WatsonAgentService_1 = __webpack_require__(/*! ./services/WatsonAgentService */ "./src/services/WatsonAgentService.ts");
let dashboardPanel;
// Supported languages
...
function activate(context) {
    ...
    (0, WatsonAgentService_1.setExtensionPath)(context.extensionPath);
...
function isSupportedLanguage(document) {
    ...
    const detectedLang = (0, pythonParser_1.detectLanguage)(fileName);
...
function updateDashboard(document) {
    ...
    const language = (0, pythonParser_1.detectLanguage)(fileName);
    console.log('[Extension] Parsing file:', fileName, 'Language:', language);
    const functions = (0, pythonParser_1.parseFunctions)(sourceCode, fileName);
...
class DashboardPanel {
    ...
    async _handleQuickFix(payload) {
        ...
        try {
            ...
            const fixedCode = await WatsonAgentService_1.watsonService.generateQuickFix(functionCode, functionName, errorMessage, inputArgs, this._currentLanguage);
    ...
    // ═══════════════════════════════════════════════════════════════
    async _handleExplainError(payload) {
        const { requestId, functionName, functionCode, errorMessage, inputArgs } = payload;
        ...
        try {
            ...
            const explanation = await WatsonAgentService_1.watsonService.explainError(functionCode, functionName, errorMessage, inputArgs, this._currentLanguage);
    ...
    // SUMMARIZE HANDLER
    // ═══════════════════════════════════════════════════════════════
    async _handleSummarize(payload) {
        const { requestId, functionName, functionCode } = payload;
        console.log('[Dashboard] Summarizing:', functionName);
        try {
            const summary = await WatsonAgentService_1.watsonService.summarizeFunction(functionCode, functionName);
            this._panel.webview.postMessage({
    ...
    // GUIDE HANDLER
    // ═══════════════════════════════════════════════════════════════
    async _handleGenerateGuide(requestId) {
        console.log('[Dashboard] Generating guide...');
        try {
            const guide = await WatsonAgentService_1.watsonService.generateGuide(this._currentFileContent, path.basename(this._currentFilePath));
            this._panel.webview.postMessage({
                type: 'guideResult',
    ...
    // EDGE CASES HANDLER
    // ═══════════════════════════════════════════════════════════════
    async _handleGenerateEdgeCases(payload) {
        const { requestId, functionName, functionCode } = payload;
        console.log('[Dashboard] Edge cases for:', functionName);
        try {
            const edgeCases = await WatsonAgentService_1.watsonService.generateEdgeCases(functionCode, functionName);
            this._panel.webview.postMessage({
    ...
    // AUDIT HANDLER
    // ═══════════════════════════════════════════════════════════════
    async _handleAuditCode(payload) {
        const { requestId, functionName, functionCode } = payload;
        console.log('[Dashboard] Auditing:', functionName);
        try {
            const auditResult = await WatsonAgentService_1.watsonService.auditCode(functionCode, functionName);
            this._panel.webview.postMessage({
                type: 'auditResult',
    ...
    async _handleApplyFix(payload) {
        ...
        const answer = await vscode.window.showWarningMessage(`Apply AI fix for "${functionName}"?\nThis will replace the function code.`, { modal: true }, 'Apply Fix', 'Cancel');
        if (answer !== 'Apply Fix')
            return;
        try {
            vscode.window.showInformationMessage('Generating fix with IBM Watsonx AI...');
            const fixedCode = await WatsonAgentService_1.watsonService.generateFix(functionCode, functionName, issue, this._currentLanguage);
    ...
    // CHAT HANDLER
    // ═══════════════════════════════════════════════════════════════
    async _handleChatQuery(payload) {
        const { requestId, functionName, functionCode, question } = payload;
        console.log('[Dashboard] Chat:', functionName);
        try {
            const response = await WatsonAgentService_1.watsonService.chatAboutCode(functionCode, functionName, question);
            this._panel.webview.postMessage({
                type: 'chatResult',
    ...
    // ═══════════════════════════════════════════════════════════════
    async _executeFunction(payload) {
    ...
    _processArgs(args) {
        return args.map((arg) => {
            ...
            if (arg.startsWith('[') || arg.startsWith('{'))
                return arg;
            if (arg === 'null' || arg === 'None' || arg === 'nil' || arg === 'true' || arg === 'false')
                return arg;
            return `"${arg}"`;
        });
    }
    async _executePython(functionName, args, filePath) {
...
`;
    ...
    async _executeJava(functionName, args, filePath) {
        const processedArgs = this._processArgs(args);
        const argsStr = processedArgs.join(', ');
...
`;
    ...
    async _executeJavaScript(functionName, args, filePath) {
        const processedArgs = this._processArgs(args);
        const argsStr = processedArgs.join(', ');
        const dirPath = path.dirname(filePath);
        const jsCode = `
...
`;
    ...
    // ═══════════════════════════════════════════════════════════════
    async _executeC(functionName, args, filePath) {
        const processedArgs = this._processArgs(args);
        const argsStr = processedArgs.join(', ');
        const dirPath = path.dirname(filePath);
...
`;
    ...
    // C++ EXECUTION - Using G++
    // ═══════════════════════════════════════════════════════════════
    async _executeCpp(functionName, args, filePath) {
        const processedArgs = this._processArgs(args);
        const argsStr = processedArgs.join(', ');
        const dirPath = path.dirname(filePath);
        const timestamp = Date.now();
...
`;
    ...
    // GO EXECUTION
    // ═══════════════════════════════════════════════════════════════
    async _executeGo(functionName, args, filePath) {
        const processedArgs = this._processArgs(args);
        const argsStr = processedArgs.join(', ');
        const dirPath = path.dirname(filePath);
        const timestamp = Date.now();
...
`;
    ...
    _extractFunctionSignature(functionName, lang) {
    ...
    _generateCCall(functionName, argsStr) {
        return `
...
`;
    ...
    updateFunctions(fileName, filePath, functions, fileContent, language = 'python') {
        this._currentFilePath = filePath;
        this._currentFileContent = fileContent;
        this._currentLanguage = language;
        const isConfigured = WatsonAgentService_1.watsonService.isConfigured();
...
/***/ "./src/services/WatsonAgentService.ts"
/*!********************************************!*\
  !*** ./src/services/WatsonAgentService.ts ***!
  \********************************************/
(__unused_webpack_module, exports, __webpack_require__) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.watsonService = exports.WatsonAgentService = void 0;
...
class WatsonAgentService {
    ...
    async queryAgent(systemPrompt, userContent) {
        const config = this.getConfig();
        if (!config)
            throw new Error('IBM Watsonx not configured');
    ...
    // ═══════════════════════════════════════════════════════════════
    async explainError(functionCode, functionName, errorMessage, inputArgs, language) {
...
Return ONLY valid JSON (no other text):
{
  ...
  "suggestion": "Try: [give specific example values that will work, like 'hello' for name or 5 for n]"
    ...
    // PROJECT GUIDE
    // ═══════════════════════════════════════════════════════════════
    async generateGuide(fullCode, fileName) {
...
Return your response as valid JSON only:
{
  ...
  "dependencies": ["List dependencies"]
    ...
    // SUMMARIZER
    // ═══════════════════════════════════════════════════════════════
    async summarizeFunction(functionCode, functionName) {
...
Return as valid JSON only:
{
  ...
  "complexity": "Time/space complexity notes"
    ...
    // ═══════════════════════════════════════════════════════════════
    async generateEdgeCases(functionCode, functionName) {
...
Provide 4-6 test cases with high severity ones first.`;
    ...
    // ═══════════════════════════════════════════════════════════════
    async auditCode(functionCode, functionName) {
...
Return as valid JSON:
{
  ...
  "issues": [
    ...
    // CODE FIXER
    // ═══════════════════════════════════════════════════════════════
    async generateFix(functionCode, functionName, issue, language) {
...
Make sure the code is valid ${language} syntax.`;
    ...
    // CHAT
    // ═══════════════════════════════════════════════════════════════
    async chatAboutCode(functionCode, functionName, question) {
    ...
    // ═══════════════════════════════════════════════════════════════
    async generateQuickFix(functionCode, functionName, errorMessage, inputArgs, language) {
...
exports.WatsonAgentService = WatsonAgentService;
exports.watsonService = WatsonAgentService.getInstance();
 ...
 */
function parseFunctions(sourceCode, fileName) {
...
// GO PARSER
// ═══════════════════════════════════════════════════════════════
function parseGoFunctions(sourceCode) {
    const functions = [];
    const lines = sourceCode.split('\n');
...
// C/C++ PARSER
// ═══════════════════════════════════════════════════════════════
function parseCFunctions(sourceCode, language) {
    const functions = [];
    const lines = sourceCode.split('\n');
...
// ═══════════════════════════════════════════════════════════════
function parseJSFunctions(sourceCode, language) {
    const functions = [];
    const lines = sourceCode.split('\n');
